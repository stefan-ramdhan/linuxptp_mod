...
int msg_post_recv(struct ptp_message *m, int cnt)
{
	...
	case PDELAY_RESP:
		...
		//stefan add - on master side, we don't process pdelay_resp.
		//printf("ts of pdelay_resp (s) = %" PRIu64 "\n", m->ts.pdu.sec);
		//printf("ts of pdelay_resp (ns) = %" PRIu32 "\n", m->ts.pdu.nsec);
		//exit(EXIT_FAILURE);
		//printf("with sequenceID of %" PRIu16 "\n", m->header.sequenceId);
		//stefan end
...
int msg_pre_send(struct ptp_message *m)
{
	...
	case PDELAY_REQ:
		break;
	case PDELAY_RESP: //pdelayresp is processed before sending.
