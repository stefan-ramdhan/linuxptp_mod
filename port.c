...
int process_pdelay_req(struct port *p, struct ptp_message *m)
{
	...
	if (p->timestamping == TS_P2P1STEP) {
		...
		rsp->pdelay_resp.requestReceiptTimestamp = //where ingress TS of pdelayreq is stored into pdelayresp. network byte order (big)
			tmv_to_Timestamp(m->hwts.ts);

		/*
		//stefan add
		uint64_t reqRecTS = ((unsigned long)rsp->pdelay_resp.requestReceiptTimestamp.seconds_msb << 32) + rsp->pdelay_resp.requestReceiptTimestamp.seconds_lsb;
		printf("from receipt (s) = %lu\n", reqRecTS);
		printf("from receipt (ns) = %u\n", rsp->pdelay_resp.requestReceiptTimestamp.nanoseconds);
		printf("seqID = %u\n\n", rsp->pdelay_resp.hdr.sequenceId);
		//stefan end
		*/
	}
